    ...
    <div class="grid-container">
       ...
       <?php
            if(isset($_GET["err"]) && !empty($_GET["err"])){
                $error = $_GET["err"];
                echo '<span style="color:red;padding-left:20px;">'.$error.'</span>';
            }
        ?>
        <div class="grid-container">
            <ul class="tabs" data-tabs id="example-tabs">
                <li class="tabs-title is-active"><a href="#search-browser" aria-selected="true">Reservations</a>
                </li>
                <?php                           
                    if (isset($email)){
                            ...
                            <?php
                                if (isset($email)){ 
                                    if ($_SESSION['role'] == "Admin"){
                                        ...
                                        if(empty($_GET["search"])){ 
                                            $sql = "SELECT * FROM Reservations WHERE CustomerName = '$email'";
                                            $result = mysqli_query($link, $sql);
                                        }else{
                                            $search = $_GET["search"];
                                            $filter = $_GET["filter"];
                                            
                                            $sql1 = "SELECT * FROM Reservations WHERE $filter LIKE ? AND CustomerName = '$email'";
                                    ...
                                    if(isset($result)){                   
                                        if(mysqli_num_rows($result) > 0){
                                            if (!(empty($err))){
                                                echo '<span style="color:red;padding-left:20px;">'. $err .'</span><br>';
                                            }
                    ...
                    if (isset($email)){
                        if ($_SESSION['role'] == "Admin"){
                            echo '<div class="tabs-panel"  id="search-forms">';
                                echo '<form action="../ResfileSystem.php" method="POST" enctype="multipart/form-data">';
